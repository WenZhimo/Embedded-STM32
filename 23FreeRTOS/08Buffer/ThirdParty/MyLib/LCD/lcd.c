...
void lcd_init(void)
{
    ...
    g_sram_handle.Init.NSBank = FMC_NORSRAM_BANK1;                        /* 根据 F429 原理图，改为使用 NE1 */
    ...
    /* FMC读时序控制寄存器 */
    fmc_read_handle.AddressSetupTime = 0x0F;            /* 地址建立时间(ADDSET) */
    fmc_read_handle.AddressHoldTime = 0x00;             /* 地址保持时间(ADDHLD) 模式A没有用到 */
    fmc_read_handle.DataSetupTime = 60;                 /* 数据保存时间(DATAST) */
    fmc_read_handle.AccessMode = FMC_ACCESS_MODE_A;     /* 模式A */
    
    /* FMC写时序控制寄存器 */
    fmc_write_handle.AddressSetupTime = 9;              /* 地址建立时间(ADDSET) */
    fmc_write_handle.AddressHoldTime = 0x00;            /* 地址保持时间(ADDHLD) 模式A没有用到 */
    fmc_write_handle.DataSetupTime = 9;                 /* 数据保存时间(DATAST) */
    fmc_write_handle.AccessMode = FMC_ACCESS_MODE_A;    /* 模式A */
    
    HAL_SRAM_Init(&g_sram_handle, &fmc_read_handle, &fmc_write_handle);
    HAL_Delay(50);  /* 替换了原子的 delay_ms */
    ...
    {
        /* 重新配置写时序控制寄存器的时序 */
        fmc_write_handle.AddressSetupTime = 3; /* 地址建立时间(ADDSET)为3个fsmc_ker_ck=6*3=18ns */
        fmc_write_handle.DataSetupTime = 3;    /* 数据保持时间(DATAST)为3个fsmc_ker_ck=6*3=18ns */
    ...
    {
        /* 重新配置写时序控制寄存器的时序 */
        fmc_write_handle.AddressSetupTime = 2; /* 地址建立时间(ADDSET)为2个fsmc_ker_ck=6*2=12ns */
        fmc_write_handle.DataSetupTime = 2;    /* 数据保持时间(DATAST)为2个fsmc_ker_ck=6*2=12ns */
        FMC_NORSRAM_Extended_Timing_Init(g_sram_handle.Extended, &fmc_write_handle, g_sram_handle.Init.NSBank, g_sram_handle.Init.ExtendedMode);
    }
    else if (lcddev.id == 0x5310 || lcddev.id == 0x7796 || lcddev.id == 0x1963)
    {
        /* 重新配置写时序控制寄存器的时序 */
        fmc_write_handle.AddressSetupTime = 1; /* 地址建立时间(ADDSET)为1个fsmc_ker_ck=6*1=6ns */
        fmc_write_handle.DataSetupTime = 1;    /* 数据保持时间(DATAST)为1个fsmc_ker_ck=6*1=6ns */
        FMC_NORSRAM_Extended_Timing_Init(g_sram_handle.Extended, &fmc_write_handle, g_sram_handle.Init.NSBank, g_sram_handle.Init.ExtendedMode);
    }
