...
uint8_t myGetKeyPressStateByID(KeyID_t keyid)
{
    ...
    key_state = HAL_GPIO_ReadPin(keyGPIOPorts[keyid], keyGPIOPins[keyid]) == (GPIO_PinState)keyActiveLevels[keyid];
    #ifdef USE_RTOS
    ...
    if(key_state == (HAL_GPIO_ReadPin(keyGPIOPorts[keyid], keyGPIOPins[keyid]) == (GPIO_PinState)keyActiveLevels[keyid])){
        return key_state;
    } else {
