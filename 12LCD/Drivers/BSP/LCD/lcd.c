...
FSMC 接口可以通过地址信号，快速地找到存储器对应存储块上的数据。

STM32F1 的 FSMC 将外部存储器划分为四个存储块（每个固定大小为 256M 字节），
每个存储块又被分为 4 个区，每个区管理 64M 字节空间，每个区都有独立的寄存器对所连接的存储器进行配置。
 ...
 * @retval      无
 */
uint16_t Hex2RGB565(uint32_t hexColor) {
    // 提取R、G、B通道值
...
/**
* @brief       判断字符长度，并把字符长度和数据存储在结构体里返回，用于函数判断
 * @param       chr  : 要判断的字符
 * @retval      无
 */
CHAR_LENGTH my_lcd_charlength(char* chr){
    CHAR_LENGTH result;
    uint8_t i = 0;
#ifdef LCD_GB2312
    if((chr[0] & 0x80) != 0x00){           /* 是汉字，双字节 */
        if(chr[1]=='\0')return;                     /* 错误 */
        result.Length = 2;
        result.chr[0] = chr[0];
        result.chr[1] = chr[1];
        result.chr[2] = '\0';
...
/**
* @brief       在指定位置显示一个字符，本函数参考江协的 OLED显示函数以及正点原子的LCD显示函数
 * @param       x,y  : 坐标
 * @param       chr  : 要显示的字符:" "--->"~"
 * @param       size : 字体大小 12/16/24/32,目前只有16
 * @param       mode : 叠加方式(1); 非叠加方式(0);
 * @param       color : 字符的颜色;
 * @retval      无
 */
void my_lcd_show_char(uint16_t x, uint16_t y, char *chr, uint8_t size, uint8_t mode, uint16_t color)
{
    ...
    uint16_t y0 = y;
    uint8_t csize = 32;
    const CC_DATA *pfont = 0;
    if(size==16){
        pfont = HAN_16;
    }else if(size==12) {
        /* something……?懒得整12像素的汉字 */
        //pfont = HAN_12;
    }else if(size==32){
        pfont = HAN_32;
    }
/* 判断字符编码，并将字符数据转存到SingleChar和CharLength里 */
    CHAR_LENGTH result = my_lcd_charlength(chr);
    CharLength = result.Length;
    SingleChar = result.chr;
    //printf("%s\n",SingleChar);
    
    csize = (size / 8 + ((size % 8) ? 1 : 0)) * (size / 2) * (CharLength==1?1:2);
    
    uint16_t pIndex =0;
    
    for (pIndex = (CharLength==1?0:95); strcmp(pfont[pIndex].Index, "") != 0; pIndex ++)
    {
        /*找到匹配的字符*/
        if (strcmp(pfont[pIndex].Index, SingleChar) == 0)
        {
            break;      //跳出循环，此时pIndex的值为指定字符的索引
        }
    }
    //printf("%s:%d\n",pfont[pIndex].Index,csize);
    for (t = 0; t < csize; t++)
    {
        temp = pfont[pIndex].Data[t];    /* 获取字符的点阵数据 */
...
/**
* @brief       在指定位置显示一个字符串，本函数参考正点原子的LCD显示函数
 * @param       x,y  : 坐标
 * @param       string : 要显示的字符
 * @param       size : 字体大小 12/16/24/32,目前只有16
 * @param       mode : 叠加方式(1); 非叠加方式(0);
 ...
 * @retval      无
 */
void my_lcd_show_string(uint16_t x, uint16_t y, char *String, uint8_t size, uint8_t mode, uint16_t color)
{
    //printf("%s\n",String);
    uint16_t i = 0;
    char *SingleChar;
    ...
    {
        CHAR_LENGTH result = my_lcd_charlength(&String[i]);
        CharLength = result.Length;
        SingleChar = result.chr;
        i+=CharLength;
        my_lcd_show_char(x,y,SingleChar,size,mode,color);
        x+=(CharLength==1?1:2)*size/2;
    }
}
 ...
 * @retval      无
 */
void my_lcd_show_image(uint16_t x, uint16_t y, uint16_t w, uint16_t h, const unsigned char *image_data){
    uint32_t i,j; 
    uint8_t picH,picL; 
    //lcd_set_window(x,y,x+w-1,y+h-1);//窗口设置
    for(i=0;i<h;i++)
    {
        for(j=0;j<w;j++){
            uint32_t n = i*w+j ;
